document.addEventListener('DOMContentLoaded', () => {
    ...
    imageUpload.addEventListener('change', (e) => {
        handleSingleImageFiles(e.target.files);
    });
    ...
    function handleBatchImageFiles(fileList) {
    ...
    splitImageUpload.addEventListener('change', (e) => {
        handleSplitImageFiles(e.target.files);
    });

    function handleSplitImageFiles(files) {
        const file = files[0];
        if (!file) return;
    ...
    // 初始化拖拽上传
    setupDragAndDrop(singleUpload, handleSingleImageFiles);
    setupDragAndDrop(batchUpload, handleBatchImageFiles);
    setupDragAndDrop(splitUpload, handleSplitImageFiles);
    ...
    function setupDragAndDrop(dropZone, fileHandler) {
        ...
        dropZone.addEventListener('click', (e) => {
            if (e.target.tagName === 'INPUT' || e.target.closest('.upload-label')) {
                return;
            }
    ...
    processBtn.addEventListener('click', () => {
        if (currentMode === 'single') {
            ...
            // 检查是否需要去除背景
            // 检查尺寸是否包含连字符（矩形尺寸）
            const isRectangle = selectedSize.toString().indexOf('-') !== -1;

            if (removeBg) {
                if (isRectangle) {
                    const [w, h] = selectedSize.split('-').map(num => parseInt(num));
                    resizeAndRemoveBgRectangle(originalFile, w, h, quality, threshold);
                } else {
                ...
                if (isRectangle) {
                    const [w, h] = selectedSize.split('-').map(num => parseInt(num));
                    resizeImageToRectangle(originalFile, w, h, quality);
                } else {
                ...
                img.onload = () => {
                    try {
                        ...
                        if (isRectangle) {
                            const [w, h] = selectedSize.split('-').map(num => parseInt(num));
                            if (removeBg) {
                                resizeAndRemoveBgRectangleForBatch(file, img, w, h, quality, threshold)
                                    .then(result => resolve(result))
                                    .catch(error => reject(error));
                            } else {
                                resizeImageToRectangleForBatch(file, img, w, h, quality)
                                    .then(result => resolve(result))
                                    .catch(error => reject(error));
                            }
                        } else {
                            ...
                            if (removeBg) {
                                resizeAndRemoveBgForBatch(file, img, size, quality, threshold)
                                    .then(result => resolve(result))
                                    .catch(error => reject(error));
                            } else {
                                resizeImageForBatch(file, img, size, quality)
                                    .then(result => resolve(result))
                                    .catch(error => reject(error));
                            }
                        }
